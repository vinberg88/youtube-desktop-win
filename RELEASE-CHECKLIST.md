...
- [ ] Check Store package metadata: display name, publisher, application ID, background color, package identity alignment, and supported languages (`en-US` only).
- [ ] Verify required assets are present and current (`assets/icon.ico`, Store screenshots, privacy policy URL, listing copy).
- [ ] Confirm Store listing copy, submission notes, release notes, and screenshot text are English only.
